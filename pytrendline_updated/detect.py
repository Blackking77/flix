...
def get_pivots(
  ...
  pseries = candlestick_data.df[col][scan_from_index:].reset_index(drop=True)
  pivots = set([])
...
def detect(
  ...
  def detect_wrapped(tt):
    ...
    if scan_from_date != None:
      scan_from_index = int(candlestick_data.df.loc[candlestick_data.df["Date"] == scan_from_date].index[0])
      print(scan_from_index)
      print(type(scan_from_index))
    else:
        ...
        for k in range(i, len(pseries)):
          ...
          if not is_breakout:
            if tt == structs.TrendlineTypes.RESISTANCE and trend_price_at_k < pseries[k] - breakout_tolerance or \
              ...
              breakout_index = k
              breakout_date = candlestick_data.df.iloc[k].Date
              is_breakout = True
